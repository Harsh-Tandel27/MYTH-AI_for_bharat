...
export async function POST(request: NextRequest) {
  try {
    ...
    const creditCost = isEdit ? CREDIT_COSTS.CHAT_EDIT : CREDIT_COSTS.PROMPT_GENERATION;
    const creditDesc = isEdit ? 'Chat Edit — AI code modification' : 'Prompt Generation — AI website generation';
    const creditResult = await requireCredits(creditCost, creditDesc);
...
IMAGE MATCHING FOR WEBSITE CLONES (CRITICAL):
- Scraped content includes an IMAGE CATALOG grouped by section (PRODUCT:IPHONE, LOGO, HERO, PROMO/ENTERTAINMENT, etc.)
- ALWAYS match images to their correct sections using the catalog labels and URL paths:
  - URL containing /iphone/ → iPhone section ONLY
  - URL containing /macbook/ → MacBook section ONLY
  - URL containing /ipad/ → iPad section ONLY
  - PROMO/ENTERTAINMENT images → entertainment/streaming sections ONLY, NEVER in product sections
  - LOGO images → header/nav ONLY
- NEVER put a TV show or movie image in a product section (iPhone, iPad, Mac)
- When no scraped image matches a section, use: https://source.unsplash.com/800x600/?{exact+product+name}
  - CORRECT: "iphone+16+pro" for an iPhone section
  - WRONG: generic "technology" or "phone" for any section
- Each section must have a DIFFERENT image - never reuse the same image across sections
- Add referrerPolicy="no-referrer" and loading="lazy" to all <img> tags
- NEVER add crossOrigin="anonymous" to <img> tags — it breaks images from CDNs that don't send CORS headers

MAGIC UI & 21ST.DEV ANIMATED COMPONENT LIBRARY (USE THESE!):

The sandbox has framer-motion, clsx, and tailwind-merge pre-installed. You MUST use
animated components inspired by Magic UI and 21st.dev to make every generated site
look premium and polished. CREATE these as inline utility components in a shared file
(src/components/ui/magic.jsx) or directly inside the component that uses them.

REQUIRED: Use at least 3-5 of these effects in every website you generate:

1. MARQUEE - Infinite scrolling strip for logos, testimonials, partners:
\`\`\`jsx
...
const Marquee = ({ children, direction = "left", speed = 25 }) => (
  <div className="overflow-hidden [mask-image:linear-gradient(to_right,transparent,white_10%,white_90%,transparent)]">
    <motion.div className="flex gap-8 w-max"
...
2. BORDER BEAM - Animated gradient border that rotates around a card:
\`\`\`jsx
const BorderBeam = ({ children, className = "" }) => (
  <div className={\`relative rounded-xl p-[1px] overflow-hidden \${className}\`}>
    <div className="absolute inset-0 rounded-xl">
      <div className="absolute inset-[-200%] animate-[spin_4s_linear_infinite] bg-[conic-gradient(from_0deg,transparent_0_340deg,#3b82f6_360deg)]" />
    </div>
    <div className="relative bg-gray-900 rounded-xl p-6">{children}</div>
  </div>
);
\`\`\`

3. SHIMMER BUTTON - Button with animated shimmer/shine effect:
\`\`\`jsx
const ShimmerButton = ({ children, className = "" }) => (
  <button className={\`relative overflow-hidden rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-3 text-white font-semibold transition-transform hover:scale-105 \${className}\`}>
...
4. ANIMATED TEXT - Text that reveals character by character or word by word:
\`\`\`jsx
import { motion } from "framer-motion";
...
5. FLOATING PARTICLES - Ambient particles floating in background:
\`\`\`jsx
import { motion } from "framer-motion";
...
6. BENTO GRID - Asymmetric card grid with hover animations:
\`\`\`jsx
import { motion } from "framer-motion";
const BentoCard = ({ children, className = "", span = "" }) => (
  <motion.div
    className={\`bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-6 \${span} \${className}\`}
    whileHover={{ scale: 1.02, borderColor: "rgba(59,130,246,0.5)" }}
    transition={{ duration: 0.2 }}>
    {children}
  </motion.div>
);
// Usage: <div className="grid grid-cols-3 gap-4">
//   <BentoCard span="col-span-2 row-span-2">Hero card</BentoCard>
//   <BentoCard>Small card</BentoCard>
// </div>
\`\`\`

7. ANIMATED CARD - Card with 3D tilt/glow on hover:
\`\`\`jsx
...
const GlowCard = ({ children, className = "" }) => {
  ...
  return (
    <motion.div className={\`relative bg-gray-800 border border-gray-700 rounded-2xl p-6 \${className}\`}
...
8. SPOTLIGHT / AURORA EFFECT - Background gradient that follows the mouse:
\`\`\`jsx
...
const Spotlight = ({ children }) => {
  ...
  return (
    <div className="relative overflow-hidden"
      ...
      <motion.div className="pointer-events-none absolute -inset-px rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"
        style={{ background: \`radial-gradient(600px circle at \${mouseX}px \${mouseY}px, rgba(59,130,246,0.1), transparent 40%)\` }} />
      {children}
    </div>
  );
};
...
9. GRADIENT MESH BACKGROUND - Animated multi-color gradient blobs:
\`\`\`jsx
const GradientMesh = () => (
  <div className="absolute inset-0 overflow-hidden -z-10">
    ...
    <div className="absolute top-0 -right-40 w-80 h-80 bg-blue-500/30 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-2000" />
    <div className="absolute -bottom-40 left-20 w-80 h-80 bg-pink-500/30 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-4000" />
  </div>
);
// Add to tailwind.config.js extend: { animation: { blob: "blob 7s infinite" }, keyframes: { blob: { "0%": { transform: "translate(0,0) scale(1)" }, "33%": { transform: "translate(30px,-50px) scale(1.1)" }, "66%": { transform: "translate(-20px,20px) scale(0.9)" }, "100%": { transform: "translate(0,0) scale(1)" } } } }
// BUT since we cant edit tailwind config, use inline framer-motion animation instead
\`\`\`

10. STAGGER FADE IN - Children appear one after another:
\`\`\`jsx
import { motion } from "framer-motion";
...
MANDATORY ANIMATION USAGE IN GENERATED WEBSITES:
- Hero sections MUST use AnimatedText + Particles or GradientMesh background
- Feature/product grids MUST use BentoGrid or GlowCard with stagger animations
- Testimonials/logos MUST use Marquee for continuous scrolling
- CTA buttons MUST use ShimmerButton
- All cards MUST have hover animations (scale, border glow, or 3D tilt)
- Section reveals MUST use StaggerContainer + StaggerItem for scroll-triggered animation
- Backgrounds should use Particles, GradientMesh, or Spotlight for depth
- Since tailwind.config.js CANNOT be edited, define all custom animations using framer-motion or CSS @keyframes in index.css
